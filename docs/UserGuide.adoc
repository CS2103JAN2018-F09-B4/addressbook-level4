...
`add` `addp` `addappttopet` `sortc` `sortp` `delete` `deletep` `rmpetfrom` `rmapptfrompet` `edit` `editp` `editappt` `clear`
`schedule`
====
...
|rmpetfrom |`rmpetfrom n/CLIENT_INDEX pn/PET_NAME` |Removes a pet from a client.
