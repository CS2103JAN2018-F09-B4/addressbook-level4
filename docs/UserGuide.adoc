...
=== Removing a pet from a client: `removepetfrom`

Command format: `removepetfrom c/CLIENT_INDEX p/PET_INDEX` +

The `removepetfrom` command will remove the association between the pet and the client at the specified
index. +

[horizontal]
*CLIENT INDEX*::
The client's index in the program based on the client list. This must be supplied by the user.

*PET INDEX*::
The pet's index in the program based on the pet list. This must be supplied by the user.

Here is an example on using the `removepetfrom` command:

* `removepetfrom c/1 p/1`
If you execute this command, it will remove the association between the first pet and the first client
in their respective listing.
...
`add` `addp` `addappttopet` `sortc` `sortp` `delete` `deletep` `removepetfrom` `rmapptfrompet` `edit` `editp` `editappt` `clear`
`schedule`
====
...
|removepetfrom |`removepetfrom n/CLIENT_INDEX pn/PET_INDEX` |Removes a pet from a client.
